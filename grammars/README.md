...
dependency on one would have pulled dozens of MB of grammars for languages this
project does not support (see CLAUDE.md, "What NOT to do"). Only these five are
needed.

The `tree-sitter-wasms` package (the direct one, not the `@repomix` fork below)
was also tested and **fails to load** with `web-tree-sitter` 0.26 for all of
these grammars — its build is against an older tree-sitter ABI.

## Provenance

Four grammars are extracted from
[`@vscode/tree-sitter-wasm`](https://www.npmjs.com/package/@vscode/tree-sitter-wasm)
version **0.3.1**, `wasm/` directory, unmodified. `@vscode/tree-sitter-wasm`
does not ship a PHP grammar, so `tree-sitter-php.wasm` is extracted instead
from [`@repomix/tree-sitter-wasms`](https://www.npmjs.com/package/@repomix/tree-sitter-wasms)
version **0.1.17** (`out/tree-sitter-php.wasm`, unmodified), a maintained fork
of the `tree-sitter-wasms` project rebuilt against a current tree-sitter ABI —
verified independently below rather than trusted on the fork's say-so, exactly
as required for a grammar that decides an EXTERNAL/INTERNAL/UNRESOLVED call on
a user's real imports.
...
| `tree-sitter-php.wasm` | 1.02 MB | [tree-sitter-php](https://github.com/tree-sitter/tree-sitter-php) via `@repomix/tree-sitter-wasms` | MIT |

### ABI compatibility

All five grammars were verified to load against `web-tree-sitter` 0.26 before
...
This check is not academic. The `tree-sitter-wasms` bundle noted above loads
fine in isolation but throws inside `Language.load` with 0.26 for every
grammar tried, including PHP — compiled against an older ABI. The `@repomix`
fork was checked the same way (`Language.load` + a real `<?php … ?>` parse
producing the expected `namespace_use_declaration` / `require_once_expression`
node shapes) before being trusted, per the same discipline as the Week 3
Python check.
