...
export const EXTRACT_SYSTEM_PROMPT = [
  ...
  '',
  'You will be given one document. Report every statement in it that makes a',
  'claim about the architecture. There are two kinds and you must report both.',
  '',
  'The first kind constrains how parts of the codebase may depend on each',
  'other. Report these under one of the four relations below.',
  '',
  'The second kind says something architectural that an import graph cannot',
  'decide: coding style, review process, runtime behaviour, technology choice,',
  'or a description of what the code currently does. Report these with',
  'relation "not-checkable" and an uncheckableReason. Do not skip them — the',
  'count of statements a tool cannot check is a result in its own right, and a',
  'document that says a great deal about its architecture while stating no',
  'dependency rules is a finding, not an empty document.',
  '',
  ...
  '',
  'uncheckableReason must be one of: style-preference, process-rule,',
  'runtime-behaviour, unsupported-relation, descriptive-not-normative,',
  'technology-choice.',
  '',
  ...
  'at a file path or a module name that the document does not mention. Every',
  'statement needs a subject and an object: repeat the subject as the object',
  'for must-not-cycle, and use empty strings for a not-checkable statement.',
  'A sentence naming two subjects — "A and B must not import C" — is two',
  'statements, one per subject.',
  '',
  ...
  '',
  'Return an empty list only if the document makes no architectural claim at',
  'all — neither a dependency rule nor anything under not-checkable. A licence,',
  'a changelog or an issue template is genuinely empty. A contributing guide',
  'usually is not.',
].join('\n');
